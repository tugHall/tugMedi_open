...
You can perform parallel execution and subsequent ABC. See scripts/ABC/abc.\* in the gitHub repository for the scripts and examples.
...
# You need to chmod abc.run.sh to be executable.
# You may need to change the newline code (for Linux's or Win's).
...
-   Internally uses **a script to run in parallel**, specified by `run_script` such as `abc.01_run.R` in the configuration file.

    -   Change this script according to your purpose.

-   Internally uses `abc.run.sh`.
...
-   This script is simple. You may need to change parameters written in this script.
...
```         
<ABC>/<Sample Name>:
- param_mat.txt: Parametes         , data for abc() of library abc
